...
publicWidget.registry.WebsiteSale.include({
    ...
    _onChangeCombination: function (ev, $parent, combination) {
        ...
        if ($stickyAddToCart.length) {
            ...
            $('.tp-bottom-bar-add-to-cart').removeClass('d-none');
            if (!combination.allow_out_of_stock_order && !combination.free_qty) {
                $('.tp-bottom-bar-add-to-cart').addClass('d-none');
            }
        }
        // Discount percentage
...
publicWidget.registry.TpStickyAddToCart = animations.Animation.extend({
    ...
    _onScroll: function () {
        if (!isMobileEnv && $('#add_to_cart:visible').length) {
            if ($('#add_to_cart:visible')[0].getBoundingClientRect().top <= 0) {
                this.$el.fadeIn();
            } else {
                this.$el.fadeOut();
            }
