...
class SaleOrder(models.Model):
    ...
    def action_view_warranty(self):
        action = self.env.ref(
            "cp_warranty_management.sr_product_warranty_action"
        ).read()[0]
        action["views"] = [(self.env.ref("cp_warranty_management.sr_product_warranty_list_view").id,"list",),
                           (self.env.ref("cp_warranty_management.sr_product_warranty_form_view").id,"form",),]
