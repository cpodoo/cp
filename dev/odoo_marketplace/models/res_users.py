...
class ResUsers(models.Model):
    ...
    def config_wise_groups_status(self):
        ...
        if group_mp_product_pricelist:
            conf_group_mp_product_pricelist = self.env['ir.default']._get('res.config.settings', 'group_mp_product_pricelist')
            groups_status[group_mp_product_pricelist.id] = conf_group_mp_product_pricelist
        
        if group_mp_shop_allow:
            conf_group_mp_shop_allow = self.env['ir.default']._get('res.config.settings', 'group_mp_shop_allow')
            groups_status[group_mp_shop_allow.id] = conf_group_mp_shop_allow

        if group_mp_product_variant:            
            conf_group_mp_product_variant = self.env['ir.default']._get('res.config.settings', 'group_mp_product_variant')
            groups_status[group_mp_product_variant.id] = conf_group_mp_product_variant

        if group_mp_product_uom:
            conf_group_mp_product_uom = self.env['ir.default']._get('res.config.settings', 'group_mp_product_uom')
    ...
    def copy(self, default=None):
        ...
        if self._context.get('is_seller', False):
            ...
            wk_valse = {
                ...
                "auto_product_approve": self.env['ir.default']._get('res.config.settings', 'mp_auto_product_approve'),
                ...
                "auto_approve_qty": self.env['ir.default']._get('res.config.settings', 'mp_auto_approve_qty'),
                "seller" : True,
            }
