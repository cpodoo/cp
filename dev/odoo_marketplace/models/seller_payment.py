...
class SellerPayment(models.Model):
    ...
    def _get_mp_currency(self):
        return self.env['ir.default']._get('res.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id
    ...
    currency_id = fields.Many2one("res.currency", string="Marketplace Currency", default=_get_mp_currency,tracking=True, required=True, readonly=True)
    invoiced_amount = fields.Monetary(string='Seller Amount in seller Currency', currency_field='invoice_currency_id',
        readonly=True)
    invoice_line_ids = fields.Many2many("account.move.line", "seller_paymnet_invoice_line", "seller_payment", "account_invoice_line", "Invoice Lines", readonly=True)
    ...
    def _set_invoice_currency(self):
        self.invoice_currency_id = self.invoice_id.currency_id or self.env['ir.default']._get('res.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id
