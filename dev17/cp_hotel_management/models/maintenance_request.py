...
class MaintenanceRequest(models.Model):
    ...
    def _onchange_team_id(self):
        if self.team_id:
            ...
            self.domain_partner_ids = partner_ids
            
        else:
            self.domain_partner_ids = False

        print(f"Team Changed. Domain Partner IDs set to: {self.domain_partner_ids}")

    # Important: Make sure your maintenance.team model has a field like:
    ...
    def action_verify(self):
        self.state = 'done'
        if self.vehicle_maintenance_id and self.vehicle_maintenance_id._name == 'fleet.vehicle':
            available_state = self.env['fleet.vehicle.state'].search([('name', '=ilike', 'Available')], limit=1)
            if available_state:
                self.vehicle_maintenance_id.write({'state_id': available_state.id})
