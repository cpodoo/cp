    ...
    def get_details(self):
        ...
        for rec in check_outs:
            ...
            staff = self.env['res.users'].search_count(
                [('groups_id', 'in',
                  [self.env.ref('cp_hotel_management.hotel_group_admin').id,
                   self.env.ref(
                       'cp_hotel_management.cleaning_team_group_head').id,
                   self.env.ref(
                       'cp_hotel_management.cleaning_team_group_user').id,
                   self.env.ref(
                       'cp_hotel_management.hotel_group_reception').id,
                   self.env.ref(
                       'cp_hotel_management.maintenance_team_group_leader').id,
                   self.env.ref(
                       'cp_hotel_management.maintenance_team_group_user').id
                   ])])
