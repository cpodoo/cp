...
class SalesTarget(models.Model):
    ...
    ], string='Target Achieve', required=True)
    ...
    achieve = fields.Integer(string='Achieve', required=True)
    achieve_percentage = fields.Float(string='Achieve Percentage', compute='_compute_achieve_percentage', store=True)
    responsible_salesperson_id = fields.Many2one('res.users', string='Responsible Salesperson', required=True)
    ...
    def _compute_product_order_lines(self):
        for record in self:
            record.product_order_lines = self.env['sale.order.line'].search([
                ('target_id', '=', record.id)
            ])
    ...
    def create(self, vals_tree):
        ...
        return super(SalesTarget, self).create(vals_tree)
    ...
    def action_discard(self):
        ...
        for record in self:
            # Reset the state to 'draft' and discard changes
            record.state = 'draft'
            # Close the form view
            return {
