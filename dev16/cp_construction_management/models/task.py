...
class ProjectTask(models.Model):
    ...
    move_ids = fields.Many2many(
        ...
        string='Stock Moves',  # Added a string for clarity
        store=True,
    )
    ...
    def _compute_stock_picking_moves(self):
        for rec in self:
            stock_move_ids = []
            for picking in rec.picking_ids:
                stock_move_ids.extend(picking.move_ids.ids)
            rec.move_ids = stock_move_ids
    ...
    planning_ids = fields.One2many(
        'cp_construction_management.material_conceptions',
        'task_id',
        string='Material Requisition',
    ) #

    material_req_ids = fields.One2many('cp_construction_management.material_req', 'task_id')
    ...
    #     'cp_construction_management.material_req_line',
    #     'task_id',
    #     string='Material Requisition',
    ...
    def material_req_create(self):
        action = self.env.ref('cp_construction_management.material_req_action')
        result = action.read()[0]
    ...
    def view_notes(self):
        for rec in self:
            res = self.env.ref('cp_construction_management.action_task_note_note')
            res = res.read()[0]
