...
class MaterialRequisition(models.Model):
    _name = "cp_construction_management.material_req"
    _description = "Material Requisition"

    name = fields.Char(string='Number', readonly=True, copy=False, default='New')

    task_user_id = fields.Many2one('res.users', related='task_id.analytic_account_id', string='Task / Job Order User')
    ...
    material_req_line_ids = fields.One2many('cp_construction_management.material_req_line', 'material_reqisition_id')
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('cp_construction_management.material_req')
        result = super(MaterialRequisition, self).create(vals)

        return result

class MaterialRequisitionLine(models.Model):
    _name = "cp_construction_management.material_req_line"
    _description = "Material Requisition Line"

    material_reqisition_id = fields.Many2one('cp_construction_management.material_req', string='Material Requistion')
    ...
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env['res.company']._company_default_get())
    task_id = fields.Many2one('project.task', string='Task / Job Order')
    plan_id = fields.Many2one('cp_construction_management.material_conceptions', string='Plan')
...
class MaterialConceptions(models.Model):
    _name = "cp_construction_management.material_conceptions"
    _description = "Material Conceptions"

    material_req_line_ids = fields.One2many('cp_construction_management.material_req_line', 'plan_id', string='Material Requistion')
    product_id = fields.Many2one('product.product', string="Product")
    name = fields.Char(string="Description")
    quantity = fields.Float('Quantity', default=1)
    product_uom = fields.Many2one(
