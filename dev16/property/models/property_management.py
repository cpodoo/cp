...
class PropertyManagement(models.Model):
    ...
    property_type_id = fields.Many2one(
        "property.management.type", string="property Type"
    )

    property_contractor_id = fields.Many2one(
        "property.management.contractor", string="Contractor"
    )

    tag_ids = fields.Many2many(
        "property.management.tag", string="property Tags"
    )

    offer_ids = fields.One2many(
        "property.management.offer", "property_id", string="Offers"
    )

    payment_ids = fields.One2many(
        "property.management.payment", "property_id", string="Payments"
    )
    ...
    def property_management_action_complete(self):
        for record in self:
    ...
    def property_management_action_fail(self):
        for record in self:
    ...
    def create(self, vals):
        seq_value = self.env["ir.sequence"].next_by_code(
            "property.management.contract_id"
        )
