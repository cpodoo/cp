...
class InstallmentAnalyticLineReport(models.Model):
    _name = 'installment.analytic.line.report'
    _description = "Installment Analytic Line Report"
    _auto = False
    _rec_name = "date"
    ...
    def init(self):
        tools.drop_view_if_exists(self._cr, 'installment_analytic_line_report')
        self._cr.execute("""
          CREATE OR REPLACE VIEW installment_analytic_line_report AS
          SELECT el.id,
                 el.name,
                 el.employee_id,
                 el.date,
