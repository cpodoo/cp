...
class employee_loan(models.Model):
    ...
    def action_open_reject_wizard(self):
        ...
        elif self.state == 'dep_approval':
            rejection_source = 'hr'

        if not rejection_source:
            raise UserError(("Cannot determine the rejection source for the current loan state ('%s').", self.state))
        ...
        return {
            ...
            'name': ('Provide Reject Reason'), 
            'res_model': 'reject.reason',
            'view_mode': 'form',
            'view_id': self.env.ref('cp_hr_loan.reject_reason_wizard_view_form').id,
            'target': 'new',
    ...
    def send_loan_detail(self):
        if self.employee_id and self.employee_id.work_email:
            template_id = self.env['ir.model.data']._xmlid_to_res_id('cp_hr_loan.employee_loan_detail_send_mail')
    ...
    loan_document_line_ids = fields.One2many('cp.loan.document', 'loan_id')
    ...
    employee_partner_id = fields.Many2one('res.partner', string="Employee Partner", related='employee_id.partner_id', store=True)
    ...
    def get_loan_url(self):
        for loan in self:
            ...
            action_id = self.env.ref('cp_hr_loan.action_employee_loan').id
            menu_id = self.env.ref('cp_hr_loan.menu_employee_loan').id
            if base_url:
    ...
    def onchange_employee_id(self):
        if self.employee_id:
            ...
            dept_manager_id = self.department_id.manager_id.id if self.department_id and self.department_id.manager_id else False
            parent_id = self.employee_id.parent_id.id if self.employee_id and self.employee_id.parent_id else False
            self.manager_id = dept_manager_id or parent_id or False

            self.job_id = self.employee_id.job_id.id if self.employee_id.job_id else False

    def action_send_request(self):
        if not self.manager_id:
            raise ValidationError(('Please Select Department manager'))
        ...
        if self.manager_id and self.manager_id.work_email:
            ...
            print(ir_model_data._xmlid_lookup('cp_hr_loan.cp_manager_request'))
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.cp_manager_request')[1]
    ...
    def get_hr_manager_email(self):
        ...
        email = ''
        if group:
            # FIX: replace "users" with "user_ids" (Odoo 19)
            ...
            for emp in employee_ids:
                if emp.work_email:
                    email = (email + ',' if email else '') + emp.work_email

        return email

    def cp_manager_approval_loan(self):
        self.state = 'dep_approval'
        email = self.get_hr_manager_email()
        if email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.cp_hr_manager_request')[1]
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id)
    ...
    def hr_manager_approval_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.hr_manager_confirm_loan')[1]
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id)
            template_id.write({'email_to': self.employee_id.work_email})
            template_id.send_mail(self.ids[0], True)

    def cp_manager_reject_loan(self):
        self.state = 'reject'
        if self.employee_id.work_email:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.cp_loan_manager_reject')[1]
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id)
            template_id.write({'email_to': self.employee_id.work_email})
            template_id.send_mail(self.ids[0], True)
    ...
    def action_close_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.hr_manager_closed_loan')
            mtp = self.env['mail.template']
    ...
    def hr_manager_reject_loan(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('cp_hr_loan.hr_loan_manager_reject_loan')
            mtp = self.env['mail.template']
            template_id = mtp.browse(template_id[1])
            template_id.write({'email_to': self.employee_id.work_email})
            template_id.send_mail(self.ids[0], True)
    ...
    def paid_loan(self):
        if not self.employee_partner_id:
            raise ValidationError("Employee Partner is not linked with the Employee !!!")
        ...
        acc_move_id = self.env['account.move'].create(acc_move_vals)
        ...
        if acc_move_id:
            ...
            if self.interest_amount:
                ...
                line_vals.append(interest_val)

            debit_account = self.employee_partner_id.property_account_payable_id.id
            debit_amount = self.loan_amount + (self.interest_amount or 0.0)
            ...
            line_vals.append(debit_val)

            acc_move_id.line_ids = line_vals
    ...
    def unlink(self):
        for loan in self:
            if loan.state != 'draft':
                raise ValidationError(('Loan delete in draft state only !!!'))
        return super(employee_loan, self).unlink()

    def action_view_loan_installment(self):
        action = self.env.ref('cp_hr_loan.action_installment_line').read()[0]

        installment = self.mapped('installment_lines')
        if len(installment) > 1:
            action['domain'] = [('id', 'in', installment.ids)]
        elif installment:
            action['views'] = [(self.env.ref('cp_hr_loan.view_loan_emi_form').id, 'form')]
