...
class ChecklistActivities(models.Model):
    ...
    def _compute_check_reset_after_completed(self):
        ...
        for obj in self:
            obj.can_reset_after_completed = Param.sudo().get_param('cp_task_subtask_checklist.checklist_allow_reset_after_completed')
            obj.can_reset_after_canceled = Param.sudo().get_param('cp_task_subtask_checklist.checklist_allow_reset_after_canceled')
...
class Task(models.Model):
    ...
    def check_restriction_setting(self, new_stage):
        if new_stage.checklist_task_progress_restriction:
            if self.env.user.has_group('cp_task_subtask_checklist.group_checklist_task_progress_restriction'):
                return True
        return False
